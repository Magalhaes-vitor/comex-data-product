...
class GoldBuilder:
    ...
    def build(self):
        ...
        if any(df is None or df.empty for df in [df_aps, df_bacen, df_mdic, df_conab]):
            msg = "Um ou mais arquivos da Silver não foram encontrados ou estão vazios."
            logger.error(msg)
        ...
        try:
            ...
            logger.error(f"Arquivo de Referência não encontrado em {ref_path}")
            return False
        ...
        if sucesso_aps and sucesso_mdic and sucesso_agro:
            logger.info(f"Sucesso! Fato APS: {len(df_gold_aps)} reg. | Fato MDIC: {len(df_gold_mdic)} reg. | Fato Agro: {len(df_gold_agro)} reg.")
            return True
        else:
            return False
