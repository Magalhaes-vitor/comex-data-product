...
# Garante que o Python encontre a pasta src/ tanto localmente quanto no AWS Lambda (/var/task)
project_root = os.environ.get("LAMBDA_TASK_ROOT", os.path.dirname(os.path.abspath(__file__)))
...
def lambda_handler(event, context):
    """Ponto de entrada oficial para o AWS Lambda ou EventBridge."""
    ...
    return {
        "statusCode": 200, 
        "body": f"Pipeline concluído com sucesso em {tempo_formatado}"
    }

if __name__ == "__main__":
    # Permite rodar localmente pelo terminal simulando a injeção do Lambda
